...
- `--engine [native|legacy]`: Defaults to `native` — the fail-closed pipeline built on `src/pictova/engine/*`. `legacy` still runs the old orchestrator but does not refuse an asset the quality gate rejected.
