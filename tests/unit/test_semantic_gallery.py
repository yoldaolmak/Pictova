...
def test_append_media_to_post_content_creates_gallery(
    ...
    media_items = [
        {"media_id": 1, "url": "http://example.com/url1.jpg", "heading": "Akyaka Nerede", "heading_level": 2, "gallery": True},
        {"media_id": 2, "url": "http://example.com/url2.jpg", "heading": "Akyaka Nerede", "heading_level": 2, "gallery": True},
    ]
...
@patch.object(YOWordPressUploader, 'fetch_post_context')
@patch('requests.Session.post')
def test_append_media_to_post_content_keeps_unrequested_images_individual(
    mock_post, mock_fetch, mock_env, tmp_path, monkeypatch,
):
    """Three requested images do not silently become a gallery."""
    monkeypatch.setenv("PICTOVA_POST_MANIFEST_DIR", str(tmp_path / "manifests"))
    wp = YOWordPressUploader()
    base_post = {"content_raw": "<!-- wp:heading -->\n<h2>Akyaka Nerede</h2>\n<!-- /wp:heading -->"}

    def fetch_after_write(*_args, **_kwargs):
        if mock_post.call_args:
            return {"content_raw": mock_post.call_args.kwargs["json"]["content"], "modified": "2026-08-05T10:00:00"}
        return base_post

    mock_fetch.side_effect = fetch_after_write
    mock_post.return_value.raise_for_status = MagicMock()
    result = wp.append_media_to_post_content(124, [
        {"media_id": 1, "url": "https://example.test/1.webp", "heading": "Akyaka Nerede", "heading_level": 2},
        {"media_id": 2, "url": "https://example.test/2.webp", "heading": "Akyaka Nerede", "heading_level": 2},
        {"media_id": 3, "url": "https://example.test/3.webp", "heading": "Akyaka Nerede", "heading_level": 2},
    ])

    assert result["success"] is True
    content = mock_post.call_args.kwargs["json"]["content"]
    assert "<!-- wp:gallery" not in content
    assert content.count("<!-- wp:image") == 3
