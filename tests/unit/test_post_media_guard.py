...
def test_remove_empty_image_blocks_strips_placeholders():
    uploader = object.__new__(YOWordPressUploader)
    uploader.site = "gezievreni"
    ...
    cleaned, removed = uploader._remove_empty_image_blocks(content)

    assert removed == 1
    assert "wp:image" not in cleaned
    assert "<p>Korunan metin</p>" in cleaned
...
def test_remove_managed_media_items_keeps_retained_manifest_items(tmp_path, monkeypatch):
    monkeypatch.setenv("PICTOVA_POST_MANIFEST_DIR", str(tmp_path))
    content = (
        '<!-- wp:image {"id":1} --><img class="wp-image-1"/><!-- /wp:image -->\n'
        '<!-- wp:image {"id":2} --><img class="wp-image-2"/><!-- /wp:image -->'
    )
    save_post_media_manifest(
        site="yoldaolmak",
        post_id=99,
        media_items=[
            {"media_id": 1, "url": "https://example.com/1.webp", "file": "one.webp", "heading": "One"},
            {"media_id": 2, "url": "https://example.com/2.webp", "file": "two.webp", "heading": "Two"},
        ],
        content_raw=content,
    )
    uploader = object.__new__(YOWordPressUploader)
    uploader.site = "yoldaolmak"
    uploader.fetch_post_context = MagicMock(return_value={"content_raw": content, "modified": "2026-08-05"})
    uploader._commit_post_content = MagicMock(return_value={"success": True})
    uploader.delete_media = MagicMock(return_value={"success": True})

    result = uploader.remove_managed_media_items(99, [2])

    assert result["success"] is True
    assert result["removed_media_ids"] == [2]
    assert result["deleted_media_ids"] == [2]
    new_content = uploader._commit_post_content.call_args.kwargs["new_content"]
    remaining = uploader._commit_post_content.call_args.kwargs["media_items"]
    assert "wp-image-1" in new_content and "wp-image-2" not in new_content
    assert [item["media_id"] for item in remaining] == [1]
    uploader.delete_media.assert_called_once_with(2)
