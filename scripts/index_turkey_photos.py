...
# Dosya adı geriye dönük uyumluluk için korunuyor; indeksleyici artık tüm
# arşivi kapsıyor, yalnızca Türkiye'yi değil.
...
def _apply_migrations(con: sqlite3.Connection) -> None:
    """Apply additive schema changes without hiding unrelated SQLite errors."""
...
def _classify_photo(photo: Any) -> tuple[str, int, str]:
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="İncelenecek en fazla Photos kaydı; 0=tüm arşiv",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    print("📸 Photos Library yükleniyor (fotoğraf + video)...")
    ...
    print("\n🔍 Tüm arşiv taranıyor (Kıta → Ülke → Şehir)...")
    for i, photo in enumerate(all_items):
        ...
        row = {
            ...
            "source_path": path,
            "folder_path": str(p.parent) if p else "",
            "filename": p.name if p else photo.uuid,
            "file_extension": p.suffix.lower() if p else "",
            "width": photo.width,
            "height": photo.height,
